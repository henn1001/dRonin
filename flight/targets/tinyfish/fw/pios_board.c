...
void PIOS_Board_Init(void)
{
	...
	if (boot_count < 3) {
		...
		HwTinyFISHSetDefaults(HwTinyFISHHandle(), 0);
	...
	HwTinyFISHUSB_VCPPortGet(&hw_usb_vcpport);

	if (!usb_cdc_present) {
		/* Force VCP port function to disabled if we haven't advertised VCP in our USB descriptor */
		hw_usb_vcpport = HWTINYFISH_USB_VCPPORT_DISABLED;
	}
	...
	HwTinyFISHUSB_HIDPortGet(&hw_usb_hidport);

	if (!usb_hid_present) {
		/* Force HID port function to disabled if we haven't advertised HID in our USB descriptor */
		hw_usb_hidport = HWTINYFISH_USB_HIDPORT_DISABLED;
	}
	...
	HwTinyFISHDSMxModeOptions hw_DSMxMode;
	HwTinyFISHDSMxModeGet(&hw_DSMxMode);

	uint8_t hw_uart3;
	HwTinyFISHUart3Get(&hw_uart3);
	...
	HwTinyFISHGyroRangeOptions hw_gyro_range;
	HwTinyFISHGyroRangeGet(&hw_gyro_range);
	switch(hw_gyro_range) {
		case HWTINYFISH_GYRORANGE_250:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_250_DEG);
			break;
		case HWTINYFISH_GYRORANGE_500:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_500_DEG);
			break;
		case HWTINYFISH_GYRORANGE_1000:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_1000_DEG);
			break;
		case HWTINYFISH_GYRORANGE_2000:
			PIOS_MPU_SetGyroRange(PIOS_MPU_SCALE_2000_DEG);
			break;
	}

	HwTinyFISHAccelRangeOptions hw_accel_range;
	HwTinyFISHAccelRangeGet(&hw_accel_range);
	switch(hw_accel_range) {
		case HWTINYFISH_ACCELRANGE_2G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_2G);
			break;
		case HWTINYFISH_ACCELRANGE_4G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_4G);
			break;
		case HWTINYFISH_ACCELRANGE_8G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_8G);
			break;
		case HWTINYFISH_ACCELRANGE_16G:
			PIOS_MPU_SetAccelRange(PIOS_MPU_SCALE_16G);
			break;
	}

	// the filter has to be set before rate else divisor calculation will fail
	HwTinyFISHGyroLPFOptions hw_mpu_gyro_dlpf;
	HwTinyFISHGyroLPFGet(&hw_mpu_gyro_dlpf);
	uint16_t gyro_bandwidth =
		(hw_mpu_gyro_dlpf == HWTINYFISH_GYROLPF_188) ? 188 :
		(hw_mpu_gyro_dlpf == HWTINYFISH_GYROLPF_98)  ?  98 :
		(hw_mpu_gyro_dlpf == HWTINYFISH_GYROLPF_42)  ?  42 :
		(hw_mpu_gyro_dlpf == HWTINYFISH_GYROLPF_20)  ?  20 :
		(hw_mpu_gyro_dlpf == HWTINYFISH_GYROLPF_10)  ?  10 :
		(hw_mpu_gyro_dlpf == HWTINYFISH_GYROLPF_5)   ?   5 :
		184;
	PIOS_MPU_SetGyroBandwidth(gyro_bandwidth);

	HwTinyFISHAccelLPFOptions hw_mpu_accel_dlpf;
	HwTinyFISHAccelLPFGet(&hw_mpu_accel_dlpf);
	uint16_t acc_bandwidth = 
		(hw_mpu_accel_dlpf = HWTINYFISH_ACCELLPF_218) ? 218 :
		(hw_mpu_accel_dlpf = HWTINYFISH_ACCELLPF_99)  ?  99 :
		(hw_mpu_accel_dlpf = HWTINYFISH_ACCELLPF_45)  ?  45 :
		(hw_mpu_accel_dlpf = HWTINYFISH_ACCELLPF_21)  ?  21 :
		(hw_mpu_accel_dlpf = HWTINYFISH_ACCELLPF_10)  ?  10 :
		(hw_mpu_accel_dlpf = HWTINYFISH_ACCELLPF_5)   ?   5 :
		184;
